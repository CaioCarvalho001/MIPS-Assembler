...
int verificarTipo(char* OPcode){
    int i = 0;
    for(i = 0; i < 7; i++){
        if(!strcasecmp(tabelaDeInstrucoes[i].instrucao, OPcode)){
...
void tipoR(FILE* entrada, FILE* saida, char* buffer){
    ...
    if(!strcasecmp(OpcodeFunct, tabelaDeInstrucoes[5].binario)){
        int l = TRUE;
        ...
        for(int j = 0; j < 32; j++){
            if(!strcasecmp(bancoDeRegistradores[j].label, regR[2])){
        ...
        for(int i = 0; i < 3; i++){
            ...
            for(int j = 0; j < 32; j++){
                if(!strcasecmp(bancoDeRegistradores[j].label, regR[i])){
                    l = TRUE;
                    strcat(instrucao, bancoDeRegistradores[j].endereco);
                    break;
                }
...
void tipoI(FILE* entrada, FILE* saida, char* buffer){
    ...
    //Se eh um ADDI
    if(!strcasecmp(tabelaDeInstrucoes[7].binario, OpcodeFunct))
        ADDI = TRUE;

    //Se eh um SW
    else if(!strcasecmp(tabelaDeInstrucoes[8].binario, OpcodeFunct)) 
        SW = TRUE;

    //Se eh um LW
    else if(!strcasecmp(tabelaDeInstrucoes[9].binario, OpcodeFunct))
        LW = TRUE;
    
    //Se eh um BNE ou BEQ
    else if(!strcasecmp(tabelaDeInstrucoes[10].binario, OpcodeFunct) || !strcasecmp(tabelaDeInstrucoes[11].binario, OpcodeFunct)) 
        desvio = TRUE;
    ...
    for(int i = 0; i < 2; i++){
        ...
        //Banco de Registradores tem 32 
        for(int j = 0; j < 32; j++){
            if(!strcasecmp(bancoDeRegistradores[j].label, regI[i])){
                l = TRUE;
                strcat(instrucao, bancoDeRegistradores[j].endereco);
                break;
            }
